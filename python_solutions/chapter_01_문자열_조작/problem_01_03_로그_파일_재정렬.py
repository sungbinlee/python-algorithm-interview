...
이후, 반복문을 통해 각 로그를 확인합니다. 로그를 공백으로 분리한 후, 두 번째 단어가 
숫자인지 확인하여 숫자 로그인 경우 digits 리스트에 추가합니다. 그렇지 않은 경우에는 
문자 로그로 간주하여 letters 리스트에 추가합니다.

문자 로그인 letters 리스트를 정렬합니다. 정렬 기준은 람다 표현식으로 지정되는데, 먼저 
두 번째 단어부터 마지막까지를 기준으로 오름차순 정렬하고, 동일한 경우 첫 번째 단어를 
기준으로 오름차순 정렬합니다. 이를 위해 key 매개변수에 람다 표현식을 사용하여 정렬 
기준을 지정합니다.
