...
풀이 1:
주어진 리스트를 뒤집기 위해서는 리스트의 양쪽 끝에서부터 원소를 교환하는 방식을 사용합니다. 
- `left` 변수는 리스트의 가장 왼쪽 인덱스를 가리키고, `right` 변수는 리스트의 가장 오른쪽 인덱스를 가리킵니다.
- `left`가 `right`보다 작은 동안, 즉 리스트의 중간까지 반복하면서 `left`와 `right` 인덱스의 원소를 교환합니다.
- 교환 후에는 `left` 인덱스를 우측으로 한 칸 이동하고, `right` 인덱스를 좌측으로 한 칸 이동합니다.
