...
풀이1:
이 문제는 브루트 포스 방식으로 해결할 수 있습니다. 세 수를 모두 확인하여 합이 0이 되는 
경우를 찾습니다. 먼저 주어진 배열을 오름차순으로 정렬한 뒤, 첫 번째 숫자를 기준으로 잡고 
두 번째, 세 번째 숫자를 차례대로 확인하여 합이 0이 되는 조합을 찾습니다. 중복된 값을 
건너뛰기 위해 조건문을 추가로 사용합니다. * 시간 복잡도가 O(n^3)으로 매우 크기 때문에, 
입력이 클 경우에는 성능이 좋지 않을 수 있습니다.

풀이2:
이 문제는 투포인터 방식으로도 해결할 수 있습니다. 주어진 배열을 오름차순으로 정렬한 뒤, 
첫 번째 숫자를 기준으로 두 번째와 세 번째 숫자의 합을 조정하며 0을 찾습니다. 합이 0보다 
작으면 두 번째 숫자를 증가시켜 합을 키우고, 합이 0보다 크면 세 번째 숫자를 감소시켜 합을 
줄입니다. 만약 합이 0이 되면 결과 리스트에 추가하고, 중복된 값을 건너뛰기 위해 조건문과 
while 루프를 사용합니다. * 투포인터 방식으로 문제를 해결하므로, 시간 복잡도를 O(n^2)
로 개선할 수 있습니다.
"""
