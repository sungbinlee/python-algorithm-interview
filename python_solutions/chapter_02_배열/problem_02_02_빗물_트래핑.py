...
물이 쌓이는 원리는 변곡점을 기준으로 왼쪽과 오른쪽의 최대 높이를 비교하여 작은 쪽의 높이에서 현재 높이를 뺀 값을 물의 부피로 계산합니다.

높이를 저장하는 리스트와 좌우 포인터를 사용하여 문제를 해결합니다. 좌우 포인터를 배열의 양쪽 끝에서 시작하고, 좌우 포인터가 서로 만날 때까지 반복합니다.

각 포인터의 위치에서 최댓값을 갱신하면서 물의 부피를 계산합니다. 왼쪽 포인터와 오른쪽 포인터 중 작은 높이를 선택하여 현재 위치의 높이를 뺀 값을 물의 부피에 더합니다.

매 반복마다 물의 부피를 갱신하고, 좌우 포인터를 이동시킵니다. 최종적으로 계산된 물의 부피를 반환합니다.

풀이2: 스택을 활용하여 문제를 해결하는 방법입니다.

스택을 사용하여 변곡점을 저장하면서 물의 부피를 계산합니다.

배열의 모든 원소를 순회하면서 현재 높이와 스택의 최상단 높이를 비교합니다. 현재 높이가 스택의 최상단 높이보다 큰 경우에는 변곡점입니다.

변곡점을 만날 때마다 스택에서 변곡점을 꺼내고, 스택이 비어있지 않은 경우에는 물의 폭과 높이를 계산하여 물의 부피에 더합니다.
